...
def add_or_update_device(device_data):
    """Adiciona ou atualiza dispositivo na lista. Se for 'ac' e faltar grpc_port/host, define valores padrão."""
    global disp
    device_id = device_data.get('id')
    # Se for ar-condicionado, garantir que 'grpc_port' e 'grpc_host' estejam definidos
...
def evaluate_sensor_values():
    """
    Avalia periodicamente os valores dos sensores e aciona o sprinkler.
    Assume que as mensagens dos sensores possuem o campo 'value'.
    """
    LIMIAR_TEMPERATURA = 18.0
    ...
    while True:
        ...
        if sensor_temp and sprinkler:
            ...
            print(f"[GATEWAY] Sensor de temperatura: {temperature}. Estado desejado para sprinkler: {desired_state}")

            if sprinkler.get('state') != desired_state:
                print(f"[GATEWAY] Atualizando sprinkler '{sprinkler['id']}' para '{desired_state}'")
                success, error = send_grpc_command(sprinkler, desired_state)
                if success:
                    sprinkler['state'] = desired_state
                    print("[GATEWAY] Sprinkler atualizado com sucesso.")
                else:
                    print(f"[GATEWAY ERROR] Erro ao atualizar sprinkler: {error}")
        sleep(10)
